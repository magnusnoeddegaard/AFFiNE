            ...
            execute: async (state: GraphState) => {
              ...
              if (searchResults.length > 0) {
                ...
                const contextText = searchResults
                  .map(result => `--- ${result.metadata?.title || 'Context'} ---\n${result.content}`)
                  .join('\n\n');
                
                // Insert a system message with context before the user message
                const messagesWithContext = [...state.messages];
                const userMessageIndex = messagesWithContext.indexOf(latestUserMessage);
          ...
          config: {
            ...
            execute: async (state: GraphState) => {
              ...
              return {
                ...state,
                needsFollowUp: true,
                followUpReason: 'Additional information may be required to fully answer the query'
              };
            }
          }
        },
